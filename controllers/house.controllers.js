...
const updateHouse = expressAsyncHandler(async (req, res) => {
  try {
    ...
    const houseToUpdate = await HouseModel.findById(houseId);
...
const updateHouseImage = expressAsyncHandler(async (req, res) => {
  try {
    ...
    if (isUploader || role) {
      ...
      houseToUpdate.house_images[imageIndex].url = url;
      houseToUpdate.save();
      return res
        .status(200)
...
const deleteHouseImage = expressAsyncHandler(async (req, res) => {
  try {
    ...
    if (isUploader || role) {
      ...
      houseToUpdate.house_images.splice(imageIndex, 1);
      houseToUpdate.save();
